...
const main = async (time) => {
    ...
    try {
        ...
        await page.waitForSelector('.searchResult>.searchResult__participantName')

    
        const {index, correctName} = await page.evaluate((userInput) => {
            const elements = Array.from(document.querySelectorAll('.searchResult__participantName'));
            const idx = elements.findIndex((e) => { return e.innerText.trim().toLowerCase() === userInput.trim().toLowerCase() });
            return { index: idx, correctName: elements[idx].innerText }
        }, time)


        await Promise.all([
            await page.$$eval('.searchResult__participantName', (elements, idx) => {
